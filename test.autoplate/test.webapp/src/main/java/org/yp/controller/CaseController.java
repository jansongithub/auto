...
//        jsonObject.put("user","1");
    ...
    @RequestMapping("/searchcaseResutl.do")
    protected void searchResult(HttpServletRequest request,
                            HttpServletResponse response, ModelMap modelMap)
            throws Exception {
        List listresult = new ArrayList();
        listresult = caseDao.getTestcaseResult();

        List<CasePo> casePoList = (List<CasePo>)listresult.get(0);
        List<ResultPo> resultPoList = (List<ResultPo>)listresult.get(1);
